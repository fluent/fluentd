  ...
  module Plugin
    class Output < Base
      ...
      config_section :buffer, param_name: :buffer_config, init: true, required: false, multi: false, final: true do
        ...
        # dlq_handler for cases where buffer is corrupt and otherwise
        #   blocks message processing.
        config_param :dlq_handler, :hash, :default => { 'type' =>'drop' }, desc: 'Makes available an error handler for processing corrupt buffer chunks'
      ...
      def configure_dlq_handler
        ...
        return unless dlq_type && ['file', 'drop'].include?(dlq_type)
        
        case dlq_type.downcase
        when 'drop'
          log.debug('Configuring the DROP dead letter queue handler')
          require_relative 'dead_letter_queue_drop_handler'
          self.singleton_class.send(:include, DeadLetterQueueDropHandler)
        when 'file'
          log.debug("Configuring the File dead letter queue handler: ")
          ...
          log.debug("  Max number of DLQ files: #{shift_age}")
          log.debug("            Max file size: #{shift_size}")
          unless Dir.exists?(dir)
            Dir.mkdir(dir)
            log.debug("Created DLQ directory: '#{dir}'")
          end
          require 'logger'
          require 'json'
          ...
          log.debug ("Created DLQ file #{file}")

          require_relative 'dead_letter_queue_file_handler'
          self.singleton_class.send(:include, DeadLetterQueueFileHandler)
        end
      end
