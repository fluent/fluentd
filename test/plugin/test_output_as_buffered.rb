...
class BufferedOutputTest < Test::Unit::TestCase
  ...
  sub_test_case 'chunk feature in #write for output plugins' do
    ...
    test 'plugin is able to make use of DLQ handler on error from corrupt chunk' do
      @i = create_output(:standard)
      @i.configure(config_element('ROOT','',{},[config_element('buffer','',@hash)]))
      @i.register(:prefer_delayed_commit){ false }
      @i.instance_variable_set(:@events_from_chunk, [])
      def @i.events_from_chunk 
          @events_from_chunk
      end
      def @i.write(chunk)
          chunk.each do |t,r|
              begin
                r['message'].downcase!
                event = [t,r]
                @events_from_chunk << [:write, event]
              rescue=>e
                handle_chunk_error('sometag', e, t, r)
              end
          end
      end
      def @i.try_write(chunk)
          chunk.each do |t,r|
              begin
                r['message'].downcase!
                event << [t,r]
                @events_from_chunk << [:try_write, event]
              rescue=>e
                handle_chunk_error('sometag', e, t, r)
              end
          end
      ...
      events = [
        [event_time('2016-10-05 16:16:16 -0700'), {"message" => "\255"}],
        [event_time('2016-10-05 16:16:17 -0700'), {"message" => "yoooooooooy!"}],
      ]

      @i.emit_events("test.tag", Fluent::ArrayEventStream.new(events))

      waiting(5){ sleep 0.1 until @i.events_from_chunk.size == 1 }

      assert_equal 1, @i.events_from_chunk.size
      assert_equal :write, @i.events_from_chunk[0][0]
      matches = $log.logs.grep(/Dropping/)
      assert_equal(1, matches.length, "Logs do not contain 'Dropping' '#{$log.logs}'")
    end
