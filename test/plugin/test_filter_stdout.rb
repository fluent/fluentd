...
class StdoutFilterTest < Test::Unit::TestCase
  ...
  def test_through_record
    d = create_driver
    time = Time.now
    filtered = emit(d, {'test' => 'test'}, NanoTime.from_time(time))
  ...
  def test_output_type_json
    d = create_driver(CONFIG + "\noutput_type json")
    time = Time.now
    out = capture_log(d) { emit(d, {'test' => 'test'}, NanoTime.from_time(time)) }
  ...
  def test_output_type_hash
    ...
    d = create_driver(CONFIG + "\noutput_type hash")
    out = capture_log(d) { emit(d, {'test' => Float::NAN}, NanoTime.from_time(time)) }
  ...
  def test_include_time_key
    ...
    time = Time.now
    message_time = NanoTime.from_time(Time.parse("2011-01-02 13:14:15 UTC"))
